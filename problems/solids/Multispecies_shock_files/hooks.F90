...
subroutine hook_output(decomp,der,dx,dy,dz,outputdir,mesh,fields,mix,tsim,vizcount,x_bc,y_bc,z_bc)
    ...
    real(rkind) :: vort_pos, vort_neg, mixwidth, Al_mass, xspike, xbubbl, xspike_proc, xbubbl_proc, ep, epp, pe, det_e, det_t, det_p, curl_e, curl_t, curl_p, rcount=0.0
    ...
    associate( rho    => fields(:,:,:, rho_index), u   => fields(:,:,:,  u_index), &
       ...
       if (vizcount == 0) then
           ...
           write(outputunit,'(16A27)') 'tsim', 'mixwidth', 'vort_pos', 'vort_neg', 'Al_mass', 'xspike', 'xbubbl', 'm1_ep', 'm1_epp', 'm1_pe', 'm1_det_e', 'm1_det_t', 'm1_det_p', 'm1_curl_e', 'm1_curl_t', 'm1_curl_p'
       else
       ...
       if (nrank.eq.0) then
          ...
          !write(outputunit,'(10ES27.16E3)') tsim, mixwidth, vort_pos, vort_neg, Al_mass, xspike, xbubbl, ep, epp, pe
          write(outputunit,'(16F40.24)') tsim, mixwidth, vort_pos, vort_neg, Al_mass, xspike, xbubbl, ep, epp, pe, det_e, det_t, det_p, curl_e, curl_t, curl_p
       endif
       rcount = rcount + 1.0
       close(outputunit)
