...
contains
    subroutine init(this, inputfile )
        ...
        namelist /SINPUT/  gam, Rgas, PInf, shmod, &
                           PTeqb, pEqb, pRelax, SOSmodel, use_gTg, updateEtot, useOneG, ns, Cmu, Cbeta, Ckap, Cdiff, CY, Cdiff_g, Cdiff_gt, Cdiff_gp, Cdiff_pe, Cdiff_pe_2, &
                           x_bc1, x_bcn, y_bc1, y_bcn, z_bc1, z_bcn, &
                           strainHard, cnsrv_g, cnsrv_gt, cnsrv_gp, cnsrv_pe
        ...
        allocate(this%mix)
        call this%mix%init(this%decomp,this%der,this%fil,this%gfil,this%LAD,ns,this%PTeqb,this%pEqb,this%pRelax,SOSmodel,this%use_gTg,this%updateEtot,this%useOneG,this%strainHard,this%cnsrv_g,this%cnsrv_gt,this%cnsrv_gp,this%cnsrv_pe)
    ...
    subroutine simulate(this)
        ...
        call this%mix%getLAD(this%rho,this%e,this%sos,this%use_gTg,this%strainHard,this%x_bc,this%y_bc,this%z_bc)  ! Compute species LAD (kap, diff, diff_g, diff_gt,diff_pe)
        ! ------------------------------------------------
...
!         !mca remove this: temporary overwrite g^p with G^t for diagnostic
! do i=1,this%mix%ns
!         this%mix%material(i)%g_p(:,:,:,1) = this%mix%material(i)%g_t(:,:,:,1)*this%mix%material(i)%g_t(:,:,:,1) + this%mix%material(i)%g_t(:,:,:,4)*this%mix%material(i)%g_t(:,:,:,4) + this%mix%material(i)%g_t(:,:,:,7)*this%mix%material(i)%g_t(:,:,:,7)

!         this%mix%material(i)%g_p(:,:,:,2) = this%mix%material(i)%g_t(:,:,:,1)*this%mix%material(i)%g_t(:,:,:,2) + this%mix%material(i)%g_t(:,:,:,4)*this%mix%material(i)%g_t(:,:,:,5) + this%mix%material(i)%g_t(:,:,:,7)*this%mix%material(i)%g_t(:,:,:,8)

!         this%mix%material(i)%g_p(:,:,:,3) = this%mix%material(i)%g_t(:,:,:,1)*this%mix%material(i)%g_t(:,:,:,3) + this%mix%material(i)%g_t(:,:,:,4)*this%mix%material(i)%g_t(:,:,:,6) + this%mix%material(i)%g_t(:,:,:,7)*this%mix%material(i)%g_t(:,:,:,9)


!         this%mix%material(i)%g_p(:,:,:,4) = this%mix%material(i)%g_t(:,:,:,2)*this%mix%material(i)%g_t(:,:,:,1) + this%mix%material(i)%g_t(:,:,:,5)*this%mix%material(i)%g_t(:,:,:,4) + this%mix%material(i)%g_t(:,:,:,8)*this%mix%material(i)%g_t(:,:,:,7)

!         this%mix%material(i)%g_p(:,:,:,5) = this%mix%material(i)%g_t(:,:,:,2)*this%mix%material(i)%g_t(:,:,:,2) + this%mix%material(i)%g_t(:,:,:,5)*this%mix%material(i)%g_t(:,:,:,5) + this%mix%material(i)%g_t(:,:,:,8)*this%mix%material(i)%g_t(:,:,:,8)

!         this%mix%material(i)%g_p(:,:,:,6) = this%mix%material(i)%g_t(:,:,:,2)*this%mix%material(i)%g_t(:,:,:,3) + this%mix%material(i)%g_t(:,:,:,5)*this%mix%material(i)%g_t(:,:,:,6) + this%mix%material(i)%g_t(:,:,:,8)*this%mix%material(i)%g_t(:,:,:,9)


!         this%mix%material(i)%g_p(:,:,:,7) = this%mix%material(i)%g_t(:,:,:,3)*this%mix%material(i)%g_t(:,:,:,1) + this%mix%material(i)%g_t(:,:,:,6)*this%mix%material(i)%g_t(:,:,:,4) + this%mix%material(i)%g_t(:,:,:,9)*this%mix%material(i)%g_t(:,:,:,7)

!         this%mix%material(i)%g_p(:,:,:,8) = this%mix%material(i)%g_t(:,:,:,3)*this%mix%material(i)%g_t(:,:,:,2) + this%mix%material(i)%g_t(:,:,:,6)*this%mix%material(i)%g_t(:,:,:,5) + this%mix%material(i)%g_t(:,:,:,9)*this%mix%material(i)%g_t(:,:,:,8)

!         this%mix%material(i)%g_p(:,:,:,9) = this%mix%material(i)%g_t(:,:,:,3)*this%mix%material(i)%g_t(:,:,:,3) + this%mix%material(i)%g_t(:,:,:,6)*this%mix%material(i)%g_t(:,:,:,6) + this%mix%material(i)%g_t(:,:,:,9)*this%mix%material(i)%g_t(:,:,:,9)
...
!         !end
    ...
    subroutine get_dt(this,stability)
        ...
        real(rkind) :: dtCFL, dtmu, dtbulk, dtkap, dtdiff, dtdiff_g, dtdiff_gt, dtdiff_gp, dtplast, delta
        ...
        if ( this%CFL .LE. zero ) then
            ...
            if (this%step .LE. 20) then
               !this%dt = this%dt / 10._rkind
               this%dt = min(this%dt / 10._rkind, this%dtfixed)
               stability = 'startup'
            end if
         end if
