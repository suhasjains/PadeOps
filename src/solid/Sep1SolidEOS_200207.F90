...
contains
    ...
    subroutine plastic_deformation(this, gfull, gpfull, pe, use_gTg, rho, Temp, tdel, sxx, sxy, sxz, syy, syz, szz, mumix, yieldmix, rho0mix)
        ...
        if(present(yieldmix) .and. present(mumix) .and. present(rho0mix)) then
            ...
            rho0local = this%rho0
            !print*,this%mu,this%yield,this%melt_t,this%melt_c !mca
        endif
        ...
        do k = 1,nzp
            do j = 1,nyp
                do i = 1,nxp
                   ...
                   !if( (mulocal(i,j,k).lt.1.0d-12) .OR. (yieldlocal(i,j,k).lt.1.0d-12) ) then !mca
                   !   cycle
                   !endif
      ...
      !               !mca add plastic integration -- first order

      !               !plastic component of g
      !               g_p(1,1) = g(1,1)- gfull(i,j,k,1)
      !               g_p(1,2) = g(1,2)- gfull(i,j,k,2)
      !               g_p(1,3) = g(1,3)- gfull(i,j,k,3)
      !               g_p(2,1) = g(2,1)- gfull(i,j,k,4)
      !               g_p(2,2) = g(2,2)- gfull(i,j,k,5)
      !               g_p(2,3) = g(2,3)- gfull(i,j,k,6)
      !               g_p(3,1) = g(3,1)- gfull(i,j,k,7)
      !               g_p(3,2) = g(3,2)- gfull(i,j,k,8)
      !               g_p(3,3) = g(3,3)- gfull(i,j,k,9)

      ! !               !plastic component of Eulerian-Almansi strain tensor: ea_ij = 1/2*(I-gp_ji*gp_ij)

      ! !               !integrated plastic strain: e_p = sqrt(2/3*ea_ij*ea_ij)
      ! !               e_p(i,j,k) = e_p(i,j,k) + sqrt((                             &
      ! !          ( g_p(1,1)**2 + g_p(2,1)**2 + g_p(3,1)**2 - 1.0d0 )**2            &
      ! ! +        ( g_p(1,2)**2 + g_p(2,2)**2 + g_p(3,2)**2 - 1.0d0 )**2            &
      ! ! +        ( g_p(1,3)**2 + g_p(2,3)**2 + g_p(3,3)**2 - 1.0d0 )**2            &
      ! ! +  2.0d0*( g_p(1,1)*g_p(1,2) + g_p(2,1)*g_p(2,2) + g_p(3,1)*g_p(3,2) )**2  &
      ! ! +  2.0d0*( g_p(1,1)*g_p(1,3) + g_p(2,1)*g_p(2,3) + g_p(3,1)*g_p(3,3) )**2  &
      ! ! +  2.0d0*( g_p(1,2)*g_p(1,3) + g_p(2,2)*g_p(2,3) + g_p(3,2)*g_p(3,3) )**2  &
      ! !                                                           )/6.0d0)
  ...
  !                   !update plastic g: gp_1 = gp_0 ge_0 (ge_1)^(-1)
  ...
  !                   gpf(1,1) =                                                                                                          &
  ! - gfull(i,j,k,3)*g(2,2)*g(3,1)*gpfull(i,j,k,1) + gfull(i,j,k,2)*g(2,3)*g(3,1)*gpfull(i,j,k,1) + gfull(i,j,k,3)*g(2,1)*g(3,2)*gpfull(i,j,k,1) &
  ! - gfull(i,j,k,1)*g(2,3)*g(3,2)*gpfull(i,j,k,1) - gfull(i,j,k,2)*g(2,1)*g(3,3)*gpfull(i,j,k,1) + gfull(i,j,k,1)*g(2,2)*g(3,3)*gpfull(i,j,k,1) &
  ! - gfull(i,j,k,6)*g(2,2)*g(3,1)*gpfull(i,j,k,2) + gfull(i,j,k,5)*g(2,3)*g(3,1)*gpfull(i,j,k,2) + gfull(i,j,k,6)*g(2,1)*g(3,2)*gpfull(i,j,k,2) &
  ! - gfull(i,j,k,4)*g(2,3)*g(3,2)*gpfull(i,j,k,2) - gfull(i,j,k,5)*g(2,1)*g(3,3)*gpfull(i,j,k,2) + gfull(i,j,k,4)*g(2,2)*g(3,3)*gpfull(i,j,k,2) & 
  ! - gfull(i,j,k,9)*g(2,2)*g(3,1)*gpfull(i,j,k,3) + gfull(i,j,k,8)*g(2,3)*g(3,1)*gpfull(i,j,k,3) + gfull(i,j,k,9)*g(2,1)*g(3,2)*gpfull(i,j,k,3) &
  ! - gfull(i,j,k,7)*g(2,3)*g(3,2)*gpfull(i,j,k,3) - gfull(i,j,k,8)*g(2,1)*g(3,3)*gpfull(i,j,k,3) + gfull(i,j,k,7)*g(2,2)*g(3,3)*gpfull(i,j,k,3)

  !                   gpf(1,2) =                                                                                                          & 
  !   gfull(i,j,k,3)*g(1,2)*g(3,1)*gpfull(i,j,k,1) - gfull(i,j,k,2)*g(1,3)*g(3,1)*gpfull(i,j,k,1) - gfull(i,j,k,3)*g(1,1)*g(3,2)*gpfull(i,j,k,1) &
  ! + gfull(i,j,k,1)*g(1,3)*g(3,2)*gpfull(i,j,k,1) + gfull(i,j,k,2)*g(1,1)*g(3,3)*gpfull(i,j,k,1) - gfull(i,j,k,1)*g(1,2)*g(3,3)*gpfull(i,j,k,1) &
  ! + gfull(i,j,k,6)*g(1,2)*g(3,1)*gpfull(i,j,k,2) - gfull(i,j,k,5)*g(1,3)*g(3,1)*gpfull(i,j,k,2) - gfull(i,j,k,6)*g(1,1)*g(3,2)*gpfull(i,j,k,2) &
  ! + gfull(i,j,k,4)*g(1,3)*g(3,2)*gpfull(i,j,k,2) + gfull(i,j,k,5)*g(1,1)*g(3,3)*gpfull(i,j,k,2) - gfull(i,j,k,4)*g(1,2)*g(3,3)*gpfull(i,j,k,2) &
  ! + gfull(i,j,k,9)*g(1,2)*g(3,1)*gpfull(i,j,k,3) - gfull(i,j,k,8)*g(1,3)*g(3,1)*gpfull(i,j,k,3) - gfull(i,j,k,9)*g(1,1)*g(3,2)*gpfull(i,j,k,3) &
  ! + gfull(i,j,k,7)*g(1,3)*g(3,2)*gpfull(i,j,k,3) + gfull(i,j,k,8)*g(1,1)*g(3,3)*gpfull(i,j,k,3) - gfull(i,j,k,7)*g(1,2)*g(3,3)*gpfull(i,j,k,3) 

  !                   gpf(1,3) =                                                                                                          &
  ! - gfull(i,j,k,3)*g(1,2)*g(2,1)*gpfull(i,j,k,1) + gfull(i,j,k,2)*g(1,3)*g(2,1)*gpfull(i,j,k,1) + gfull(i,j,k,3)*g(1,1)*g(2,2)*gpfull(i,j,k,1) &
  ! - gfull(i,j,k,1)*g(1,3)*g(2,2)*gpfull(i,j,k,1) - gfull(i,j,k,2)*g(1,1)*g(2,3)*gpfull(i,j,k,1) + gfull(i,j,k,1)*g(1,2)*g(2,3)*gpfull(i,j,k,1) &
  ! - gfull(i,j,k,6)*g(1,2)*g(2,1)*gpfull(i,j,k,2) + gfull(i,j,k,5)*g(1,3)*g(2,1)*gpfull(i,j,k,2) + gfull(i,j,k,6)*g(1,1)*g(2,2)*gpfull(i,j,k,2) &
  ! - gfull(i,j,k,4)*g(1,3)*g(2,2)*gpfull(i,j,k,2) - gfull(i,j,k,5)*g(1,1)*g(2,3)*gpfull(i,j,k,2) + gfull(i,j,k,4)*g(1,2)*g(2,3)*gpfull(i,j,k,2) &
  ! - gfull(i,j,k,9)*g(1,2)*g(2,1)*gpfull(i,j,k,3) + gfull(i,j,k,8)*g(1,3)*g(2,1)*gpfull(i,j,k,3) + gfull(i,j,k,9)*g(1,1)*g(2,2)*gpfull(i,j,k,3) &
  ! - gfull(i,j,k,7)*g(1,3)*g(2,2)*gpfull(i,j,k,3) - gfull(i,j,k,8)*g(1,1)*g(2,3)*gpfull(i,j,k,3) + gfull(i,j,k,7)*g(1,2)*g(2,3)*gpfull(i,j,k,3)

  !                   gpf(2,1) =                                                                                                          &
  ! - gfull(i,j,k,3)*g(2,2)*g(3,1)*gpfull(i,j,k,4) + gfull(i,j,k,2)*g(2,3)*g(3,1)*gpfull(i,j,k,4) + gfull(i,j,k,3)*g(2,1)*g(3,2)*gpfull(i,j,k,4) &
  ! - gfull(i,j,k,1)*g(2,3)*g(3,2)*gpfull(i,j,k,4) - gfull(i,j,k,2)*g(2,1)*g(3,3)*gpfull(i,j,k,4) + gfull(i,j,k,1)*g(2,2)*g(3,3)*gpfull(i,j,k,4) &
  ! - gfull(i,j,k,6)*g(2,2)*g(3,1)*gpfull(i,j,k,5) + gfull(i,j,k,5)*g(2,3)*g(3,1)*gpfull(i,j,k,5) + gfull(i,j,k,6)*g(2,1)*g(3,2)*gpfull(i,j,k,5) &
  ! - gfull(i,j,k,4)*g(2,3)*g(3,2)*gpfull(i,j,k,5) - gfull(i,j,k,5)*g(2,1)*g(3,3)*gpfull(i,j,k,5) + gfull(i,j,k,4)*g(2,2)*g(3,3)*gpfull(i,j,k,5) &
  ! - gfull(i,j,k,9)*g(2,2)*g(3,1)*gpfull(i,j,k,6) + gfull(i,j,k,8)*g(2,3)*g(3,1)*gpfull(i,j,k,6) + gfull(i,j,k,9)*g(2,1)*g(3,2)*gpfull(i,j,k,6) &
  ! - gfull(i,j,k,7)*g(2,3)*g(3,2)*gpfull(i,j,k,6) - gfull(i,j,k,8)*g(2,1)*g(3,3)*gpfull(i,j,k,6) + gfull(i,j,k,7)*g(2,2)*g(3,3)*gpfull(i,j,k,6)

  !                   gpf(2,2) =                                                                                                          &
  !   gfull(i,j,k,3)*g(1,2)*g(3,1)*gpfull(i,j,k,4) - gfull(i,j,k,2)*g(1,3)*g(3,1)*gpfull(i,j,k,4) - gfull(i,j,k,3)*g(1,1)*g(3,2)*gpfull(i,j,k,4) &
  ! + gfull(i,j,k,1)*g(1,3)*g(3,2)*gpfull(i,j,k,4) + gfull(i,j,k,2)*g(1,1)*g(3,3)*gpfull(i,j,k,4) - gfull(i,j,k,1)*g(1,2)*g(3,3)*gpfull(i,j,k,4) &
  ! + gfull(i,j,k,6)*g(1,2)*g(3,1)*gpfull(i,j,k,5) - gfull(i,j,k,5)*g(1,3)*g(3,1)*gpfull(i,j,k,5) - gfull(i,j,k,6)*g(1,1)*g(3,2)*gpfull(i,j,k,5) &
  ! + gfull(i,j,k,4)*g(1,3)*g(3,2)*gpfull(i,j,k,5) + gfull(i,j,k,5)*g(1,1)*g(3,3)*gpfull(i,j,k,5) - gfull(i,j,k,4)*g(1,2)*g(3,3)*gpfull(i,j,k,5) &
  ! + gfull(i,j,k,9)*g(1,2)*g(3,1)*gpfull(i,j,k,6) - gfull(i,j,k,8)*g(1,3)*g(3,1)*gpfull(i,j,k,6) - gfull(i,j,k,9)*g(1,1)*g(3,2)*gpfull(i,j,k,6) &
  ! + gfull(i,j,k,7)*g(1,3)*g(3,2)*gpfull(i,j,k,6) + gfull(i,j,k,8)*g(1,1)*g(3,3)*gpfull(i,j,k,6) - gfull(i,j,k,7)*g(1,2)*g(3,3)*gpfull(i,j,k,6)

  !                   gpf(2,3) =                                                                                                          &
  ! - gfull(i,j,k,3)*g(1,2)*g(2,1)*gpfull(i,j,k,4) + gfull(i,j,k,2)*g(1,3)*g(2,1)*gpfull(i,j,k,4) + gfull(i,j,k,3)*g(1,1)*g(2,2)*gpfull(i,j,k,4) &
  ! - gfull(i,j,k,1)*g(1,3)*g(2,2)*gpfull(i,j,k,4) - gfull(i,j,k,2)*g(1,1)*g(2,3)*gpfull(i,j,k,4) + gfull(i,j,k,1)*g(1,2)*g(2,3)*gpfull(i,j,k,4) &
  ! - gfull(i,j,k,6)*g(1,2)*g(2,1)*gpfull(i,j,k,5) + gfull(i,j,k,5)*g(1,3)*g(2,1)*gpfull(i,j,k,5) + gfull(i,j,k,6)*g(1,1)*g(2,2)*gpfull(i,j,k,5) &
  ! - gfull(i,j,k,4)*g(1,3)*g(2,2)*gpfull(i,j,k,5) - gfull(i,j,k,5)*g(1,1)*g(2,3)*gpfull(i,j,k,5) + gfull(i,j,k,4)*g(1,2)*g(2,3)*gpfull(i,j,k,5) &
  ! - gfull(i,j,k,9)*g(1,2)*g(2,1)*gpfull(i,j,k,6) + gfull(i,j,k,8)*g(1,3)*g(2,1)*gpfull(i,j,k,6) + gfull(i,j,k,9)*g(1,1)*g(2,2)*gpfull(i,j,k,6) &
  ! - gfull(i,j,k,7)*g(1,3)*g(2,2)*gpfull(i,j,k,6) - gfull(i,j,k,8)*g(1,1)*g(2,3)*gpfull(i,j,k,6) + gfull(i,j,k,7)*g(1,2)*g(2,3)*gpfull(i,j,k,6)
 
  !                   gpf(3,1) =                                                                                                          &
  ! - gfull(i,j,k,3)*g(2,2)*g(3,1)*gpfull(i,j,k,7) + gfull(i,j,k,2)*g(2,3)*g(3,1)*gpfull(i,j,k,7) + gfull(i,j,k,3)*g(2,1)*g(3,2)*gpfull(i,j,k,7) &
  ! - gfull(i,j,k,1)*g(2,3)*g(3,2)*gpfull(i,j,k,7) - gfull(i,j,k,2)*g(2,1)*g(3,3)*gpfull(i,j,k,7) + gfull(i,j,k,1)*g(2,2)*g(3,3)*gpfull(i,j,k,7) &
  ! - gfull(i,j,k,6)*g(2,2)*g(3,1)*gpfull(i,j,k,8) + gfull(i,j,k,5)*g(2,3)*g(3,1)*gpfull(i,j,k,8) + gfull(i,j,k,6)*g(2,1)*g(3,2)*gpfull(i,j,k,8) &
  ! - gfull(i,j,k,4)*g(2,3)*g(3,2)*gpfull(i,j,k,8) - gfull(i,j,k,5)*g(2,1)*g(3,3)*gpfull(i,j,k,8) + gfull(i,j,k,4)*g(2,2)*g(3,3)*gpfull(i,j,k,8) &
  ! - gfull(i,j,k,9)*g(2,2)*g(3,1)*gpfull(i,j,k,9) + gfull(i,j,k,8)*g(2,3)*g(3,1)*gpfull(i,j,k,9) + gfull(i,j,k,9)*g(2,1)*g(3,2)*gpfull(i,j,k,9) &
  ! - gfull(i,j,k,7)*g(2,3)*g(3,2)*gpfull(i,j,k,9) - gfull(i,j,k,8)*g(2,1)*g(3,3)*gpfull(i,j,k,9) + gfull(i,j,k,7)*g(2,2)*g(3,3)*gpfull(i,j,k,9)

  !                   gpf(3,2) =                                                                                                          &
  !   gfull(i,j,k,3)*g(1,2)*g(3,1)*gpfull(i,j,k,7) - gfull(i,j,k,2)*g(1,3)*g(3,1)*gpfull(i,j,k,7) - gfull(i,j,k,3)*g(1,1)*g(3,2)*gpfull(i,j,k,7) &
  ! + gfull(i,j,k,1)*g(1,3)*g(3,2)*gpfull(i,j,k,7) + gfull(i,j,k,2)*g(1,1)*g(3,3)*gpfull(i,j,k,7) - gfull(i,j,k,1)*g(1,2)*g(3,3)*gpfull(i,j,k,7) &
  ! + gfull(i,j,k,6)*g(1,2)*g(3,1)*gpfull(i,j,k,8) - gfull(i,j,k,5)*g(1,3)*g(3,1)*gpfull(i,j,k,8) - gfull(i,j,k,6)*g(1,1)*g(3,2)*gpfull(i,j,k,8) &
  ! + gfull(i,j,k,4)*g(1,3)*g(3,2)*gpfull(i,j,k,8) + gfull(i,j,k,5)*g(1,1)*g(3,3)*gpfull(i,j,k,8) - gfull(i,j,k,4)*g(1,2)*g(3,3)*gpfull(i,j,k,8) &
  ! + gfull(i,j,k,9)*g(1,2)*g(3,1)*gpfull(i,j,k,9) - gfull(i,j,k,8)*g(1,3)*g(3,1)*gpfull(i,j,k,9) - gfull(i,j,k,9)*g(1,1)*g(3,2)*gpfull(i,j,k,9) &
  ! + gfull(i,j,k,7)*g(1,3)*g(3,2)*gpfull(i,j,k,9) + gfull(i,j,k,8)*g(1,1)*g(3,3)*gpfull(i,j,k,9) - gfull(i,j,k,7)*g(1,2)*g(3,3)*gpfull(i,j,k,9)

  !                   gpf(3,3) =                                                                                                          &
  ! - gfull(i,j,k,3)*g(1,2)*g(2,1)*gpfull(i,j,k,7) + gfull(i,j,k,2)*g(1,3)*g(2,1)*gpfull(i,j,k,7) + gfull(i,j,k,3)*g(1,1)*g(2,2)*gpfull(i,j,k,7) &
  ! - gfull(i,j,k,1)*g(1,3)*g(2,2)*gpfull(i,j,k,7) - gfull(i,j,k,2)*g(1,1)*g(2,3)*gpfull(i,j,k,7) + gfull(i,j,k,1)*g(1,2)*g(2,3)*gpfull(i,j,k,7) &
  ! - gfull(i,j,k,6)*g(1,2)*g(2,1)*gpfull(i,j,k,8) + gfull(i,j,k,5)*g(1,3)*g(2,1)*gpfull(i,j,k,8) + gfull(i,j,k,6)*g(1,1)*g(2,2)*gpfull(i,j,k,8) &
  ! - gfull(i,j,k,4)*g(1,3)*g(2,2)*gpfull(i,j,k,8) - gfull(i,j,k,5)*g(1,1)*g(2,3)*gpfull(i,j,k,8) + gfull(i,j,k,4)*g(1,2)*g(2,3)*gpfull(i,j,k,8) &
  ! - gfull(i,j,k,9)*g(1,2)*g(2,1)*gpfull(i,j,k,9) + gfull(i,j,k,8)*g(1,3)*g(2,1)*gpfull(i,j,k,9) + gfull(i,j,k,9)*g(1,1)*g(2,2)*gpfull(i,j,k,9) &
  ! - gfull(i,j,k,7)*g(1,3)*g(2,2)*gpfull(i,j,k,9) - gfull(i,j,k,8)*g(1,1)*g(2,3)*gpfull(i,j,k,9) + gfull(i,j,k,7)*g(1,2)*g(2,3)*gpfull(i,j,k,9)

                  
  !                   gpfull(i,j,k,1) = gpf(1,1); gpfull(i,j,k,2) = gpf(1,2); gpfull(i,j,k,3) = gpf(1,3)
  !                   gpfull(i,j,k,4) = gpf(2,1); gpfull(i,j,k,5) = gpf(2,2); gpfull(i,j,k,6) = gpf(2,3)
  !                   gpfull(i,j,k,7) = gpf(3,1); gpfull(i,j,k,8) = gpf(3,2); gpfull(i,j,k,9) = gpf(3,3)

  !                   gpfull(i,j,k,:) = gpfull(i,j,k,:)/detg
                    ...
                    alms(1,2) = 0.5*(1.0 - (gp(1,1)*gp(1,2) + gp(2,1)*gp(2,2) + gp(3,1)*gp(3,2)) )
                    alms(1,3) = 0.5*(1.0 - (gp(1,1)*gp(1,3) + gp(2,1)*gp(2,3) + gp(3,1)*gp(3,3)) )
                    alms(2,1) = 0.5*(1.0 - (gp(1,2)*gp(1,1) + gp(2,2)*gp(2,1) + gp(3,2)*gp(3,1)) )
                    alms(2,2) = 0.5*(1.0 - (gp(1,2)*gp(1,2) + gp(2,2)*gp(2,2) + gp(3,2)*gp(3,2)) )
                    alms(2,3) = 0.5*(1.0 - (gp(1,2)*gp(1,3) + gp(2,2)*gp(2,3) + gp(3,2)*gp(3,3)) )
                    alms(3,1) = 0.5*(1.0 - (gp(1,3)*gp(1,1) + gp(2,3)*gp(2,1) + gp(3,3)*gp(3,1)) )
                    alms(3,2) = 0.5*(1.0 - (gp(1,3)*gp(1,2) + gp(2,3)*gp(2,2) + gp(3,3)*gp(3,2)) )
                    ...
                    !update pe
                    pe(i,j,k) =  pe(i,j,k) + (almsn-almsn0)

                    ! !norm of the delta strain
                    ! almsnd = sqrt( ( (1.0 - gpdlt(1,1)**2 - gpdlt(2,1)**2 - gpdlt(3,1)**2)**2 + (1.0 - gpdlt(1,2)**2 - gpdlt(2,2)**2 - gpdlt(3,2)**2)**2 + (1.0 - gpdlt(1,3)**2 - gpdlt(2,3)**2 - gpdlt(3,3)**2)**2 )/6.0d0 + ( (-gpdlt(1,1)*gpdlt(1,2) - gpdlt(2,1)*gpdlt(2,2) - gpdlt(3,1)*gpdlt(3,2))**2 + (-gpdlt(1,1)*gpdlt(1,3) - gpdlt(2,1)*gpdlt(2,3) - gpdlt(3,1)*gpdlt(3,3))**2 + (-gpdlt(1,2)*gpdlt(1,3) - gpdlt(2,2)*gpdlt(2,3) - gpdlt(3,2)*gpdlt(3,3))**2 )/3.0d0 ) !same as solidmod
                    ...
                    ! sigman = sqrt(max(   sigma(1,1)*sigma(1,1) + sigma(1,2)*sigma(2,1) + sigma(1,3)*sigma(3,1) &
                    !                    + sigma(2,1)*sigma(1,2) + sigma(2,2)*sigma(2,2) + sigma(2,3)*sigma(3,2) &  
                    !                    + sigma(3,1)*sigma(1,3) + sigma(3,2)*sigma(2,3) + sigma(3,3)*sigma(3,3) ,1.0e-32 ))


                    ! !Lp RHS product
                    ! gpt1(1,1) = gedlt(1,1)*geinv(1,1) + gedlt(1,2)*geinv(2,1) + gedlt(1,3)*geinv(3,1)
                    ! gpt1(1,2) = gedlt(1,1)*geinv(1,2) + gedlt(1,2)*geinv(2,2) + gedlt(1,3)*geinv(3,2)     
                    ! gpt1(1,3) = gedlt(1,1)*geinv(1,3) + gedlt(1,2)*geinv(2,3) + gedlt(1,3)*geinv(3,3)
                    ! gpt1(2,1) = gedlt(2,1)*geinv(1,1) + gedlt(2,2)*geinv(2,1) + gedlt(2,3)*geinv(3,1)
                    ! gpt1(2,2) = gedlt(2,1)*geinv(1,2) + gedlt(2,2)*geinv(2,2) + gedlt(2,3)*geinv(3,2)     
                    ! gpt1(2,3) = gedlt(2,1)*geinv(1,3) + gedlt(2,2)*geinv(2,3) + gedlt(2,3)*geinv(3,3)
                    ! gpt1(3,1) = gedlt(3,1)*geinv(1,1) + gedlt(3,2)*geinv(2,1) + gedlt(3,3)*geinv(3,1)
                    ! gpt1(3,2) = gedlt(3,1)*geinv(1,2) + gedlt(3,2)*geinv(2,2) + gedlt(3,3)*geinv(3,2)     
                    ! gpt1(3,3) = gedlt(3,1)*geinv(1,3) + gedlt(3,2)*geinv(2,3) + gedlt(3,3)*geinv(3,3)
                    
                    ! !Lp norm
                    ! lpn = sqrt(max(   gpt1(1,1)*gpt1(1,1) + gpt1(1,2)*gpt1(2,1) + gpt1(1,3)*gpt1(3,1) &
                    !                 + gpt1(2,1)*gpt1(1,2) + gpt1(2,2)*gpt1(2,2) + gpt1(2,3)*gpt1(3,2) &  
                    !                 + gpt1(3,1)*gpt1(1,3) + gpt1(3,2)*gpt1(2,3) + gpt1(3,3)*gpt1(3,3) ,1.0e-32 ))

                    ! !lpn2 = sqrt(2.0/3.0* max(   gpt1(1,1)*gpt1(1,1)*alms(1,1) + gpt1(1,2)*gpt1(2,1)*alms(1,2) + gpt1(1,3)*gpt1(3,1)*alms(1,3) &
                    ! !                          + gpt1(2,1)*gpt1(1,2)*alms(2,1) + gpt1(2,2)*gpt1(2,2)*alms(2,2) + gpt1(2,3)*gpt1(3,2)*alms(2,3) &  
                    ! !                          + gpt1(3,1)*gpt1(1,3)*alms(3,1) + gpt1(3,2)*gpt1(2,3)*alms(3,2) + gpt1(3,3)*gpt1(3,3)*alms(3,3) ,1.0e-32 ))


                    !pe(i,j,k) =  pe(i,j,k) + almsn*lpn !ok
                    !pe(i,j,k) =  pe(i,j,k) + (almsn-almsn0) !better
                    !pe(i,j,k) =  pe(i,j,k) + almsnd !not stable?


                    !pe(i,j,k) =  pe(i,j,k) + almsn*(lpn**2) !not good
                    !pe(i,j,k) =  pe(i,j,k) + (almsn*lpn)**2 !not good

                    !pe(i,j,k) =  pe(i,j,k) + almsn*lpn*sigman ! not good


                    !pe(i,j,k) =  pe(i,j,k) + lpn2 !not good

                    !pe(i,j,k) =  pe(i,j,k) + (almsn*lpn !ok


                    ! !Lp RHS product
                    ! gpt2(1,1) = gedlt(1,1)*geinv(1,1) + gedlt(1,2)*geinv(1,2) + gedlt(1,3)*geinv(1,3)
                    ! gpt2(1,2) = gedlt(1,1)*geinv(2,1) + gedlt(1,2)*geinv(2,2) + gedlt(1,3)*geinv(2,3)     
                    ! gpt2(1,3) = gedlt(1,1)*geinv(3,1) + gedlt(1,2)*geinv(3,2) + gedlt(1,3)*geinv(3,3)
                    ! gpt2(2,1) = gedlt(2,1)*geinv(1,1) + gedlt(2,2)*geinv(1,2) + gedlt(2,3)*geinv(1,3)
                    ! gpt2(2,2) = gedlt(2,1)*geinv(2,1) + gedlt(2,2)*geinv(2,2) + gedlt(2,3)*geinv(2,3)     
                    ! gpt2(2,3) = gedlt(2,1)*geinv(3,1) + gedlt(2,2)*geinv(3,2) + gedlt(2,3)*geinv(3,3)
                    ! gpt2(3,1) = gedlt(3,1)*geinv(1,1) + gedlt(3,2)*geinv(1,2) + gedlt(3,3)*geinv(1,3)
                    ! gpt2(3,2) = gedlt(3,1)*geinv(2,1) + gedlt(3,2)*geinv(2,2) + gedlt(3,3)*geinv(2,3)     
                    ! gpt2(3,3) = gedlt(3,1)*geinv(3,1) + gedlt(3,2)*geinv(3,2) + gedlt(3,3)*geinv(3,3)
                    
                    ! !Lp norm
                    ! lpn2 = sqrt(max(   gpt2(1,1)*gpt2(1,1) + gpt2(1,2)*gpt2(2,1) + gpt2(1,3)*gpt2(3,1) &
                    !                  + gpt2(2,1)*gpt2(1,2) + gpt2(2,2)*gpt2(2,2) + gpt2(2,3)*gpt2(3,2) &  
                    !                  + gpt2(3,1)*gpt2(1,3) + gpt2(3,2)*gpt2(2,3) + gpt2(3,3)*gpt2(3,3) ,1.0e-32 ))




                    ! gpt2(1,1) =   gp(1,1)*gpt1(1,1)*gpinv(1,1) + gp(1,1)*gpt1(1,2)*gpinv(2,1) + gp(1,1)*gpt1(1,3)*gpinv(3,1)  &
                    !             + gp(1,2)*gpt1(2,1)*gpinv(1,1) + gp(1,2)*gpt1(2,2)*gpinv(2,1) + gp(1,2)*gpt1(2,3)*gpinv(3,1)  &
                    !             + gp(1,3)*gpt1(3,1)*gpinv(1,1) + gp(1,3)*gpt1(3,2)*gpinv(2,1) + gp(1,3)*gpt1(3,3)*gpinv(3,1)  
                    ! gpt2(1,2) =   gp(1,1)*gpt1(1,1)*gpinv(1,2) + gp(1,1)*gpt1(1,2)*gpinv(2,2) + gp(1,1)*gpt1(1,3)*gpinv(3,2)  &
                    !             + gp(1,2)*gpt1(2,1)*gpinv(1,2) + gp(1,2)*gpt1(2,2)*gpinv(2,2) + gp(1,2)*gpt1(2,3)*gpinv(3,2)  &
                    !             + gp(1,3)*gpt1(3,1)*gpinv(1,2) + gp(1,3)*gpt1(3,2)*gpinv(2,2) + gp(1,3)*gpt1(3,3)*gpinv(3,2)  
                    ! gpt2(1,3) =   gp(1,1)*gpt1(1,1)*gpinv(1,3) + gp(1,1)*gpt1(1,2)*gpinv(2,3) + gp(1,1)*gpt1(1,3)*gpinv(3,3)  &
                    !             + gp(1,2)*gpt1(2,1)*gpinv(1,3) + gp(1,2)*gpt1(2,2)*gpinv(2,3) + gp(1,2)*gpt1(2,3)*gpinv(3,3)  &
                    !             + gp(1,3)*gpt1(3,1)*gpinv(1,3) + gp(1,3)*gpt1(3,2)*gpinv(2,3) + gp(1,3)*gpt1(3,3)*gpinv(3,3)  
                    ! gpt2(2,1) =   gp(2,1)*gpt1(1,1)*gpinv(1,1) + gp(2,1)*gpt1(1,2)*gpinv(2,1) + gp(2,1)*gpt1(1,3)*gpinv(3,1)  &
                    !             + gp(2,2)*gpt1(2,1)*gpinv(1,1) + gp(2,2)*gpt1(2,2)*gpinv(2,1) + gp(2,2)*gpt1(2,3)*gpinv(3,1)  &
                    !             + gp(2,3)*gpt1(3,1)*gpinv(1,1) + gp(2,3)*gpt1(3,2)*gpinv(2,1) + gp(2,3)*gpt1(3,3)*gpinv(3,1)  
                    ! gpt2(2,2) =   gp(2,1)*gpt1(1,1)*gpinv(1,2) + gp(2,1)*gpt1(1,2)*gpinv(2,2) + gp(2,1)*gpt1(1,3)*gpinv(3,2)  &
                    !             + gp(2,2)*gpt1(2,1)*gpinv(1,2) + gp(2,2)*gpt1(2,2)*gpinv(2,2) + gp(2,2)*gpt1(2,3)*gpinv(3,2)  &
                    !             + gp(2,3)*gpt1(3,1)*gpinv(1,2) + gp(2,3)*gpt1(3,2)*gpinv(2,2) + gp(2,3)*gpt1(3,3)*gpinv(3,2)  
                    ! gpt2(2,3) =   gp(2,1)*gpt1(1,1)*gpinv(1,3) + gp(2,1)*gpt1(1,2)*gpinv(2,3) + gp(2,1)*gpt1(1,3)*gpinv(3,3)  &
                    !             + gp(2,2)*gpt1(2,1)*gpinv(1,3) + gp(2,2)*gpt1(2,2)*gpinv(2,3) + gp(2,2)*gpt1(2,3)*gpinv(3,3)  &
                    !             + gp(2,3)*gpt1(3,1)*gpinv(1,3) + gp(2,3)*gpt1(3,2)*gpinv(2,3) + gp(2,3)*gpt1(3,3)*gpinv(3,3)  
                    ! gpt2(3,1) =   gp(3,1)*gpt1(1,1)*gpinv(1,1) + gp(3,1)*gpt1(1,2)*gpinv(2,1) + gp(3,1)*gpt1(1,3)*gpinv(3,1)  &
                    !             + gp(3,2)*gpt1(2,1)*gpinv(1,1) + gp(3,2)*gpt1(2,2)*gpinv(2,1) + gp(3,2)*gpt1(2,3)*gpinv(3,1)  &
                    !             + gp(3,3)*gpt1(3,1)*gpinv(1,1) + gp(3,3)*gpt1(3,2)*gpinv(2,1) + gp(3,3)*gpt1(3,3)*gpinv(3,1)  
                    ! gpt2(3,2) =   gp(3,1)*gpt1(1,1)*gpinv(1,2) + gp(3,1)*gpt1(1,2)*gpinv(2,2) + gp(3,1)*gpt1(1,3)*gpinv(3,2)  &
                    !             + gp(3,2)*gpt1(2,1)*gpinv(1,2) + gp(3,2)*gpt1(2,2)*gpinv(2,2) + gp(3,2)*gpt1(2,3)*gpinv(3,2)  &
                    !             + gp(3,3)*gpt1(3,1)*gpinv(1,2) + gp(3,3)*gpt1(3,2)*gpinv(2,2) + gp(3,3)*gpt1(3,3)*gpinv(3,2)  
                    ! gpt2(3,3) =   gp(3,1)*gpt1(1,1)*gpinv(1,3) + gp(3,1)*gpt1(1,2)*gpinv(2,3) + gp(3,1)*gpt1(1,3)*gpinv(3,3)  &
                    !             + gp(3,2)*gpt1(2,1)*gpinv(1,3) + gp(3,2)*gpt1(2,2)*gpinv(2,3) + gp(3,2)*gpt1(2,3)*gpinv(3,3)  &
                    !             + gp(3,3)*gpt1(3,1)*gpinv(1,3) + gp(3,3)*gpt1(3,2)*gpinv(2,3) + gp(3,3)*gpt1(3,3)*gpinv(3,3)  

                    ! gpt2(1,1) =   gp(1,1)*gpt1(1,1) + gp(1,2)*gpt1(2,1) + gp(1,3)*gpt1(3,1)
                    ! gpt2(1,2) =   gp(1,1)*gpt1(1,2) + gp(1,2)*gpt1(2,2) + gp(1,3)*gpt1(3,2)                   
                    ! gpt2(1,3) =   gp(1,1)*gpt1(1,3) + gp(1,2)*gpt1(2,3) + gp(1,3)*gpt1(3,3)
                    ! gpt2(2,1) =   gp(2,1)*gpt1(1,1) + gp(2,2)*gpt1(2,1) + gp(2,3)*gpt1(3,1)
                    ! gpt2(2,2) =   gp(2,1)*gpt1(1,2) + gp(2,2)*gpt1(2,2) + gp(2,3)*gpt1(3,2)                   
                    ! gpt2(2,3) =   gp(2,1)*gpt1(1,3) + gp(2,2)*gpt1(2,3) + gp(2,3)*gpt1(3,3)
                    ! gpt2(3,1) =   gp(3,1)*gpt1(1,1) + gp(3,2)*gpt1(2,1) + gp(3,3)*gpt1(3,1)
                    ! gpt2(3,2) =   gp(3,1)*gpt1(1,2) + gp(3,2)*gpt1(2,2) + gp(3,3)*gpt1(3,2)                   
                    ! gpt2(3,3) =   gp(3,1)*gpt1(1,3) + gp(3,2)*gpt1(2,3) + gp(3,3)*gpt1(3,3)
                    
                    !almsn2 = sqrt( ( (1.0 - gpt2(1,1)**2 - gpt2(2,1)**2 - gpt2(3,1)**2)**2 + (1.0 - gpt2(1,2)**2 - gpt2(2,2)**2 - gpt2(3,2)**2)**2 + (1.0 - gpt2(1,3)**2 - gpt2(2,3)**2 - gpt2(3,3)**2)**2 )/6.0d0 + ( (-gpt2(1,1)*gpt2(1,2) - gpt2(2,1)*gpt2(2,2) - gpt2(3,1)*gpt2(3,2))**2 + (-gpt2(1,1)*gpt2(1,3) - gpt2(2,1)*gpt2(2,3) - gpt2(3,1)*gpt2(3,3))**2 + (-gpt2(1,2)*gpt2(1,3) - gpt2(2,2)*gpt2(2,3) - gpt2(3,2)*gpt2(3,3))**2 )/3.0d0 ) !same as solidmod

                    
                    !pe(i,j,k) =  pe(i,j,k) - min(   gpt2(1,1)*gpt1(1,1) + gpt2(1,2)*gpt1(2,1) + gpt2(1,3)*gpt1(3,1)    + gpt2(2,1)*gpt1(1,2) + gpt2(2,2)*gpt1(2,2) + gpt2(2,3)*gpt1(3,2)       + gpt2(3,1)*gpt1(1,3) + gpt2(3,2)*gpt1(2,3) + gpt2(3,3)*gpt1(3,3) ) , 0.0)

                    !pe(i,j,k) =  pe(i,j,k) +  (  gpt2(1,1)*gpt1(1,1) + gpt2(1,2)*gpt1(2,1) + gpt2(1,3)*gpt1(3,1)    + gpt2(2,1)*gpt1(1,2) + gpt2(2,2)*gpt1(2,2) + gpt2(2,3)*gpt1(3,2)       + gpt2(3,1)*gpt1(1,3) + gpt2(3,2)*gpt1(2,3) + gpt2(3,3)*gpt1(3,3) ) 

                    ! pe(i,j,k) =  pe(i,j,k) + (   ( alms(1,1)*gpt1(1,1) + alms(1,2)*gpt1(2,1) + alms(1,3)*gpt1(3,1) ) & 
                    !                            + ( alms(1,1)*gpt1(1,2) + alms(1,2)*gpt1(2,2) + alms(1,3)*gpt1(3,2) ) & 
                    !                            + ( alms(1,1)*gpt1(1,3) + alms(1,2)*gpt1(2,3) + alms(1,3)*gpt1(3,3) ) & 
                    !                            + ( alms(2,1)*gpt1(1,1) + alms(2,2)*gpt1(2,1) + alms(2,3)*gpt1(3,1) ) & 
                    !                            + ( alms(2,1)*gpt1(1,2) + alms(2,2)*gpt1(2,2) + alms(2,3)*gpt1(3,2) ) & 
                    !                            + ( alms(2,1)*gpt1(1,3) + alms(2,2)*gpt1(2,3) + alms(2,3)*gpt1(3,3) ) & 
                    !                            + ( alms(3,1)*gpt1(1,1) + alms(3,2)*gpt1(2,1) + alms(3,3)*gpt1(3,1) ) & 
                    !                            + ( alms(3,1)*gpt1(1,2) + alms(3,2)*gpt1(2,2) + alms(3,3)*gpt1(3,2) ) & 
                    !                            + ( alms(3,1)*gpt1(1,3) + alms(3,2)*gpt1(2,3) + alms(3,3)*gpt1(3,3) ) )


                    !if((i.eq.1).and.(j.eq.1).and.(k.eq.1)) then
                    !   print*,maxval(pe),minval(pe)
                    !endif
                         ! + gp(1,1)*gpt1(1,2) + gp(1,2)*gpt1(2,2) + gp(1,3)*gpt1(3,2) & 
                         ! + gp(1,3)*gpt1(1,2) + gp(1,2)*gpt1(2,3) + gp(1,3)*gpt1(3,3) & 
                         ! + gp(2,1)*gpt1(1,1) + gp(2,2)*gpt1(2,1) + gp(2,3)*gpt1(3,1) & 
                         ! + gp(2,1)*gpt1(1,2) + gp(2,2)*gpt1(2,2) + gp(2,3)*gpt1(3,2) & 
                         ! + gp(2,3)*gpt1(1,2) + gp(2,2)*gpt1(2,3) + gp(2,3)*gpt1(3,3) & 
                         ! + gp(3,1)*gpt1(1,1) + gp(3,2)*gpt1(2,1) + gp(3,3)*gpt1(3,1) & 
                         ! + gp(3,1)*gpt1(1,2) + gp(3,2)*gpt1(2,2) + gp(3,3)*gpt1(3,2) & 
                         ! + gp(3,3)*gpt1(1,2) + gp(3,2)*gpt1(2,3) + gp(3,3)*gpt1(3,3)



                    ! !DEBUG
                    ! !gfull = max(gfull,0.0)
                    ! !gpfull = max(gpfull,0.0)
